...
<section class="content">
	...
	<div class="col-md-12 box box-default">
		...
		<div class="box-body">
		<?php
			echo $this->Form->create("addClass",["type"=>"file","class"=>"validateForm form-horizontal","role"=>"form","id"=>"form"]);
			...
			echo '<div class="col-md-6">';
			echo @$this->Form->select("assistant_staff_member",$assistant_staff,["default"=>$data['assistant_staff_member'],"empty"=>__("Select Staff Member"),"class"=>"form-control validate[required]" ]);
			echo "</div>";
			...
			echo "<div class='form-group'>";
			echo '<label class="control-label col-md-2" for="email">'. __("Class Booking Fees").'<span class="text-danger"> *</span></label>';
			echo '<div class="col-md-6">';
			echo "<div class='input-group'>";	
			echo "<span class='input-group-addon'>".$this->Gym->get_currency_symbol()."</span>";
			echo $this->Form->input("",["label"=>false,"name"=>"class_fees","class"=>"form-control validate[required,custom[onlyNumberSp],maxSize[50]]","value"=>(($edit)?$data['class_fees']:'')]);
			echo "</div>";
			echo "</div>";
			echo "</div>";
			...
			echo '<div class="col-md-6">';
			$days = [__("Sunday")=>__("Sunday"),__("Monday")=>__("Monday"),__("Tuesday")=>__("Tuesday"),__("Wednesday")=>__("Wednesday"),__("Thursday")=>__("Thursday"),__("Friday")=>__("Friday"),__("Saturday")=>__("Saturday")];
					...
					if($edit)
					{
						...
						foreach($schedule_list as $schedule)
						{
							...
							<tr id='data'>
								...
								<?php foreach($days_time as $day){ ?>
									<input type="hidden" name="demo[]" value='[<?php echo "&quot;". $day."&quot;,&quot;".$schedule["start_time"]."&quot;,&quot;".$schedule["end_time"] ."&quot;"; ?>]' class="time_schedule">
								<?php }?>
		...
		<input type="hidden" value="<?php echo ($edit)?count($schedule_list):0 ?>" id="hidchkval">
	</div>
</section>
<?php
if(!$edit)
...
$("#add_time").click(function(){
	...
	{
		alert("<?php echo __('Please select days,start time and end time'); ?>");
		return false;
	}
	...
	if(tmp_list!=''){
		...
		alert("<?php echo __('This class schedule already added.'); ?>");
		}
});
...
function gettime(){
	...
	if(st >= nd){
		alert("<?php echo __('Plese enter valid time.'); ?> ");
		return false;
	}else{
		return true;
	}
