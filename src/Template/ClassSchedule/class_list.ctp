			...
			<table class="mydataTable table table-striped" width="100%">
				...
				foreach($data as $row)
				{
					...
					echo "<tr>
							...
							echo"<td>
								<a href='{$this->request->base}/ClassSchedule/editClass/{$row['id']}' title='".__('Edit')."' class='btn btn-flat btn-primary'><i class='fa fa-edit'></i></a>
								<a href='{$this->request->base}/ClassSchedule/deleteClass/{$row['id']}' title='".__('Delete')."' class='btn btn-flat btn-danger' onClick=\"return confirm('$confirmMsg');\"><i class='fa fa-trash-o'></i></a>
							</td>";
							}
					echo "</tr>";
				}
