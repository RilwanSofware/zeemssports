		...
		<div class="box-body">
			<form method="post" id="attendance">  
				<div class="form-group col-md-3">
					...
					<input type="text" class="form-control validate[required] sdate"  name="sdate" id="startDate"  placeholder="<?php echo  $this->Gym->get_db_format(date($this->Gym->getSettings("date_format")));?>">				
				</div>
				<div class="form-group col-md-3">
					<label for="exam_id"><?php echo __('End Date');?></label><span class='text-danger'> *</span>
					<input type="text" autocomplete="off" class="form-control validate[required] edate" name="edate" id="endDate"  placeholder="<?php echo $this->Gym->get_db_format(date($this->Gym->getSettings("date_format")));?>">					
				</div>
			...
			if(isset($_REQUEST['attendance_report']))
			{
				...
				if(isset($report_2) && empty($report_2)) {?>
					...
					<div class="clear col-md-12" style="text-align: center;">
