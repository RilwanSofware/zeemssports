	...
	<div class="col-md-12 box box-default">		
		...
		<form method="post">  
			<div class="form-group col-md-3">
				...
				value="<?php if(isset($_REQUEST['sdate'])) echo date($this->Gym->getSettings("date_format"),strtotime($_REQUEST['sdate']));/* else echo date($this->Gym->getSettings("date_format")); */?>" placeholder="<?php echo date($this->Gym->getSettings("date_format"));?>">				
			</div>
			<div class="form-group col-md-3">
				<label for="exam_id"><?php echo __('End Date');?></label>
				<input type="text"  class="form-control date edate" name="edate" 
				value="<?php if(isset($_REQUEST['edate'])) echo date($this->Gym->getSettings("date_format"),strtotime($_REQUEST['edate']));else/*  echo getdate_in_input_box(date('Y-m-d')); */?>" placeholder="<?php echo date($this->Gym->getSettings("date_format"));?>">					
			</div>
