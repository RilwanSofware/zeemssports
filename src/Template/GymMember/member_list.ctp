			...
			<?php
				foreach($data as $row) {
					...
					<td>
						...
						<a href='{$this->request->base}/GymMember/viewMember/{$row['id']}' title='".__('View')."' class='btn btn-flat btn-info'><i class='fa fa-eye'></i></a>";
					if($session["role_name"] == "administrator" )
					{	
						$confirmMsg = __("Are you sure,You want to delete this record?");
					echo " <a href='{$this->request->base}/GymMember/editMember/{$row['id']}' title='".__('Edit')."' class='btn btn-flat btn-primary'><i class='fa fa-edit'></i></a>
						<a href='{$this->request->base}/GymMember/deleteMember/{$row['id']}' title='".__('Delete')."' class='btn btn-flat btn-danger' onClick=\"return confirm('$confirmMsg');\"><i class='fa fa-trash-o'></i></a>";
					}
					...
					if($session['role_name'] == "administrator") {
					echo "<td>";
						$confirmMsg2 = __("Are you sure,you want to activate this account?");
						if($row["activated"] == 0) {
							echo "<a class='btn btn-success btn-flat' onclick=\"return confirm('$confirmMsg2');\" href='".$this->request->base ."/GymMember/activateMember/{$row['id']}'>".__('Activate')."</a>";
