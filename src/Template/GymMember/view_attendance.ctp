		...
		<div class="box-body">							    
			...
			?>
				...
				<div class="form-group col-md-3">
					...
					<input type="text" name="sdate" class="form-control validate[required]" value="<?php echo ($view)?$this->Gym->get_db_format(date($this->gym->getSettings("date_format"),strtotime($s_date))):"";?>" id="startDate" autocomplete="off" >
				</div>
				<div class="form-group col-md-3">
					<label for="exam_id"><?php echo __("End Date");?></label>
					<input type="text" name="edate" class="form-control validate[required]" value="<?php echo ($view)?$this->Gym->get_db_format(date($this->gym->getSettings("date_format"),strtotime($e_date))):"";?>" id="endDate" autocomplete="off" >
				</div>
				...
				if(!empty($data))
				{
					foreach($data as $row)
					{
						...
						<tr>
							...
							<td><?php echo $this->Gym->get_class_by_id($row['class_id']); ?></td>
