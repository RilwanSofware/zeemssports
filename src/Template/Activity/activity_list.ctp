					...
					<?php
						foreach($data as $row)
						{
							echo "<tr>
								...
								echo "<td>
										<a href='{$this->request->base}/Activity/viewActivity/{$row['id']}' title='".__('View')."' class='btn btn-flat btn-info'><i class='fa fa-eye'></i></a>&nbsp;";
										if($session["role_name"] != "member"){
										echo "<a href='{$this->request->base}/Activity/editActivity/{$row['id']}' title='".__('Edit')."' class='btn btn-flat btn-primary'><i class='fa fa-edit'></i></a>
										<a href='{$this->request->base}/Activity/deleteActivity/{$row['id']}' title='".__('Delete')."' class='btn btn-flat btn-danger' onClick=\"return confirm(msg);\"><i class='fa fa-trash-o'></i></a>";						
										}
									"</td>";		
