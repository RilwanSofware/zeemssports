			...
			<table class="mydataTable table table-striped" width="100%">
				...
				<?php
					foreach($membership_data as $membership)
					{
						...
						echo " <a href='{$this->Gym->createurl("Membership","editMembership")}/{$membership->id}' title='".__('Edit')."' class='btn btn-flat btn-primary' ><i class='fa fa-edit'></i></a>
						<a title='".__('Delete')."' did='{$membership->id}' class='del-membership btn btn-flat btn-danger' data-url='".$this->Gym->createurl("GymAjax","deleteMembership")."'><i class='fa fa-trash-o'></i></a>";
						
						//if($session["role_name"] == "administrator")
						//{ 
						echo " <a href='{$this->Gym->createurl("Membership","viewActivity")}/{$membership->id}' class='btn btn-flat btn-info'>".__("Activities")."</a>";	
						//}
						echo " <a href='{$this->Gym->createurl("Membership","activateOnStripe")}/{$membership->id}' class='btn btn-flat btn-primary' >".__('Activate On Stripe')."</a>";
