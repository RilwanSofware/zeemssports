	...
	<div class="col-md-12 box box-default">		
		...
		<form method="post" class="validateForm">  
			...
			<div class="form-group col-md-3">
				...
				echo $this->Form->select("class_id",$classes,["empty"=>__("Select Class"),"class"=>"form-control validate[required]"]);?>
			...
			<table class="table">
			   ...
			   <?php
				...
				<?php foreach($data as $row) {
					$date = $this->Gym->get_db_format(date($this->Gym->getSettings("date_format"),strtotime($attendance_date))); 
				?>
				<tr>
				<?php
				if($session["role_name"] == "member")
				{
					...
					echo "<td><?php echo __(date('l',strtotime($date)));?></td>";
