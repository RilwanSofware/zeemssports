				...
				if(!empty($data))
				{
					...
					{			
						echo "<tr>
								...
								<td>
								<a href='javascript:void(0)' class='btn btn-flat btn-info view_income_expense' title='".__('View')."' data-url='".$this->request->base ."/GymAjax/viewIncomeExpense/{$row['id']}' type='income'><i class='fa fa-eye'></i></a>
								<a href='".$this->request->base ."/MembershipPayment/incomeEdit/{$row['id']}' class='btn btn-flat btn-primary' title='".__('Edit')."'><i class='fa fa-edit'></i></a>
								<a href='".$this->request->base ."/MembershipPayment/deleteIncome/{$row['id']}' title='".__('Delete')."'class='btn btn-flat btn-danger' onclick=\"return confirm('$confirmMsg')\"><i class='fa fa-trash'></i></a>
								</td>
						</tr>";
					}
				}
