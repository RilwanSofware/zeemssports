...
$("body").on("click",".save-level",function(){
	...
	if(level != "") {	
		if(level.length<=50) {
			if(!isValid) {
				alert("<?php echo __('Special characters not allow.'); ?> ");
			}else {
			...
			var message = "<?php echo __("Please Enter Maximum 50 Chars.") ; ?>";
			alert(message);
		}
	}
	else {
		var message = "<?php echo __("Please Enter Level"); ?>"; 
		alert(message);
	}
});
...
<section class="content">
	...
	<div class="col-md-12 box box-default">		
		...
		<div class='form-group'>
			...
			<div class="col-md-6">
				<?php 
					echo $this->Form->input("",["autocomplete"=>"off","label"=>false,"name"=>"start_date","class"=>"validate[required] form-control",'id'=>'startDate']);
				?>
			</div>	
		</div>
		<div class='form-group'>
			...
			<div class="col-md-6">
				<?php 
					echo $this->Form->input("",["autocomplete"=>"off","label"=>false,"name"=>"end_date","class"=>" validate[required] form-control",'id'=>'endDate']);
				?>
			</div>	
		</div>
		<div class="form-group">
...
jQuery("body").on("click", "#add_workouttype", function(event){
		 ...
		 if(day_check == 0)
		 {
			alert("<?php echo __('Please select days.'); ?>");
			//$("#save-workout").hide();
			return false;
		 }else{
			$(".achilactiveadd").hide();
		 ...
		 if(activity_check == 0)
		 {
			alert("<?php echo __('Please select activity.'); ?>");
			return false;
		 }
		  var is_empty = 0;
		 ...
		 if(is_empty == 1)
		 {
			alert("<?php echo __('Please Fill All The Fields.'); ?>");
			$("#save-workout").hide();
...
function add_activity(activity,id)
{
	...
	if(s1 == true && r1 == true && k1 == true && t1 == true){
		...
		alert("<?php echo __('Please Enter Valid Value'); ?>");
		//break;
		(".workout-block").hide();
		$("#save-workout").hide();
	}
...
$("body").on("change",".activity_check",function(){
		 ...
		 if($(this).is(":checked"))
		{
			...
			string += '<div class="achilactiveadd "><span class="label"><?php echo __('Sets'); ?> </span><input type="text" name = "sets_' + id + '" id = "sets_' + id + '" placeholder="<?php echo __('Sets'); ?>" class="activity_value_box validate[custom[integer]]" maxLength=5></div>';
			string += '<div class="achilactiveadd"><span class="label"><?php echo __('Reps'); ?> </span> <input type="text" name = "reps_' + id + '" id = "reps_' + id + '" placeholder="<?php echo __('Reps'); ?>" class="activity_value_box validate[custom[integer]]" maxLength=5></div>';
			string += '<div class="achilactiveadd"><span class="label"> <?php echo __('KG'); ?> </span><input type="text" name = "kg_' + id + '" id = "kg_' + id + '" placeholder="<?php echo __('KG'); ?>" class="activity_value_box validate[custom[integer]]" maxLength=5></div>';
			string += '<div class="achilactiveadd"><span class="label"><?php echo __('Rest Time'); ?> </span><input type="text" name = "time_' + id + '" id = "time_' + id + '" placeholder="<?php echo __('Rest Time'); ?>" class="activity_value_box validate[custom[integer]]" maxLength=5></div>';
