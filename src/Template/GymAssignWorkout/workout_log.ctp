		...
		<table class="mydataTable table table-striped" width="100%">
			...
			foreach($data as $row)
			{
				...
				echo "<tr>
					...
					<td>
						<a href='".$this->request->base ."/GymAssignWorkout/viewWorkouts/{$row['user_id']}' class='btn btn-primary btn-flat' title='".__('View')."'><i class='fa fa-eye'></i> ".__("View Workouts")."</a> 
						<a href='".$this->request->base ."/GymAssignWorkout/deleteWorkout/{$row['user_id']}' class='btn btn-danger btn-flat' title='".__('Delete')."' onclick=\"return confirm('$confirmMsg');\"><i class='fa fa-trash'></i></a>
					</td>
				</tr>";
			}
			?>
