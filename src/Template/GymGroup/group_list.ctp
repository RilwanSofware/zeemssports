	...
	<div class="col-md-12 box box-default">		
		...
		<table class="mydataTable table table-striped">
			...
			foreach($data as $row)
			{
				...
				echo "<a href='".$this->Gym->createurl('GymGroup','editGroup')."/{$row['id']}' class='btn btn-flat btn-primary' title='".__('Edit')."'><i class='fa fa-edit'></i></a>
					<a href='".$this->Gym->createurl('GymGroup','deleteGroup')."/{$row['id']}' class='btn btn-flat btn-danger' title='".__('Delete')."' onClick=\"return confirm('$confirmMsg')\"><i class='fa fa-trash-o'></i></a>";
				}
					echo " <a href='javascript:void(0)' data-url='".$this->request->base ."/GymAjax/viewGroupMember/{$row['id']}' title='".__('View')."' class='view-grp-member btn btn-flat btn-info' id={$row['id']}><i class='fa fa-eye'></i></a>
					</td>
				</tr>
				";
			}
