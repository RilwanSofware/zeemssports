...
<script>
	...
	$(".mem_list").select2();
	$("#nutrition-form").on("keypress", function (event) {
            console.log("aaya");
...
<section class="content">
	...
	<div class="col-md-12 box box-default">		
		...
		<div class="box-body">
		<?php
			echo $this->Form->create("assignWorkout",["class"=>"validateForm form-horizontal","role"=>"form","id"=>"nutrition-form"]);
		?>
...
<script>
	...
	jQuery("body").on("click", "#add_nutrition", function(event) {
		...
		var day_check = $(".selectDayChk:checked").size();	
		 if(day_check == 0)	{
			 var message = "<?php echo __("Please select days."); ?>";
			alert(message);
		 ...
		 if(activity_check == 0) {
			var message = "<?php echo __("Please select nutrition for days."); ?>";
			alert(message);
			return false;
		 }
		var not_blank = 1;
			...
			if(check_val == 'nutrition_time') {
				activity_name = $(this).attr("id");
			 	if(activity_name == 'dinner') {
					activity_name = "<?php echo __("Dinner"); ?>";
				}
				if(activity_name == 'breakfast') {
					activity_name = "<?php echo __("Break Fast"); ?>";
				}
				if(activity_name == 'lunch') {
					activity_name = "<?php echo __("Lunch"); ?>";
				}
				if(activity_name == 'midmorningsnacks') {
					activity_name = "<?php echo __("Mid-Morning Snacks"); ?>";
				}
				if(activity_name == 'afternoonsnacks') {
					activity_name = "<?php echo __('Afternoon Snacks'); ?>";
				}
				item = {};
