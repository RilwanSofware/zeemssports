		...
		<div class="box-body">
			<table class="mydataTable table table-striped" width="100%">
				...
				{				
					...
					echo "<td>
						<a href='".$this->Gym->createurl('StaffMembers','editStaff')."/{$row['id']}' class='btn btn-flat btn-primary' title='".__('Edit')."'><i class='fa fa-edit'></i></a>
						<a href='".$this->Gym->createurl('StaffMembers','deleteStaff')."/{$row['id']}' class='btn btn-flat btn-danger' title='".__('Delete')."' onClick=\"return confirm('$confirmMsg')\"><i class='fa fa-trash-o'></i></a>
						</td>";
					}
					echo "</tr>";
				} 
