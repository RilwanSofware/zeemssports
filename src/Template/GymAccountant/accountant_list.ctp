		...
		<table class="mydataTable table table-striped" width="100%">
			...
			{				
				...
				if($session["role_name"] == "administrator" || $session["role_name"] == "staff_member"){
					...
					echo "<td>
					<a href='".$this->Gym->createurl('GymAccountant','editAccountant')."/{$row['id']}' class='btn btn-flat btn-primary' title='".__('Edit')."'><i class='fa fa-edit'></i></a>
					<a href='".$this->Gym->createurl('GymAccountant','deleteAccountant')."/{$row['id']}' class='btn btn-flat btn-danger' title='".__('Delete')."' onClick=\"return confirm('$confirmMsg')\"><i class='fa fa-trash-o'></i></a>
					</td>";
				}
				echo "</tr>
				";
